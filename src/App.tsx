...
const getFlightCategory = (ceilingFt: number, visibilityKm: number): FlightCategory => {
  const visCategory: FlightCategory =
    visibilityKm < 1.6 ? "LIFR" :
    visibilityKm < 4.8 ? "IFR" :
    visibilityKm <= 8 ? "MVFR" :
    "VFR";

  // Ceiling is only considered when it exists (BKN/OVC/VV). When there is no
  // effective ceiling, category should be driven by visibility.
  if (ceilingFt < 0) return visCategory;
  ...
  return severity[ceilingCategory] >= severity[visCategory] ? ceilingCategory : visCategory;
};
      ...
      if (metarResult?.ok) {
        ...
        if (metar) {
          ...
          if (metarCeilingFt !== null) {
            ...
            currentProcessed.cloudCeiling = ceilingFt <= 0 ? -1 : ceilingFt / 3.28084;
